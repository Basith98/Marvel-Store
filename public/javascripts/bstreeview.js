...
(function ($, window, document, undefined) {
  ...
  $.extend(bstreeView.prototype, {
    ...
    build: function (parentElement, children, depth) {
      ...
      if (depth > 0) {
        leftPadding =
          (_this.settings.indent + depth * _this.settings.indent).toString() +
          "rem;";
      }
      depth += 1;
      ...
      $.each(children, function addNodes(id, node) {
        ...
        if (node.children) {
          var treeItemStateIcon = $(templates.treeviewItemStateIcon).addClass(
            node.expanded
              ? _this.settings.expandIcon
              : _this.settings.collapseIcon
          );
        ...
        if (node.href) {
          treeItem.attr("href", `/admin/categoryDetails/${node.Id}`);
        }
        ...
        if (node.children) {
          // Node group item.
          var treeGroup = $(templates.treeviewGroupItem).attr(
            "id",
            _this.itemIdPrefix + node.nodeId
          );
  ...
  $.fn[pluginName] = function (options) {
    return this.each(function () {
      if (!$.data(this, "plugin_" + pluginName)) {
        $.data(this, "plugin_" + pluginName, new bstreeView(this, options));
      }
    });
  };
